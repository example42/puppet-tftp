...
describe 'tftp' do
  ...
  describe 'Test standard installation with monitoring and firewalling' do
    ...
    it 'should monitor the process' do
      content = catalogue.resource('monitor::process', 'tftp_process').send(:parameters)[:enable]
      content.should == true
    end
  ...
  describe 'Test decommissioning - absent' do
    ...
    it 'should stop Service[tftp]' do should contain_service('tftp').with_ensure('stopped') end
    it 'should not enable at boot Service[tftp]' do should contain_service('tftp').with_enable('false') end
  ...
  describe 'Test decommissioning - disableboot' do
    ...
    it { should contain_package('tftp').with_ensure('present') }
    it { should_not contain_service('tftp').with_ensure('present') }
    it { should_not contain_service('tftp').with_ensure('absent') }
    it 'should not enable at boot Service[tftp]' do should contain_service('tftp').with_enable('false') end
    it { should contain_file('tftp.conf').with_ensure('present') }
  ...
  describe 'Test customizations - source' do
    let(:params) { {:source => "puppet://modules/tftp/spec" , :source_dir => "puppet://modules/tftp/dir/spec" , :source_dir_purge => true } }

    it 'should request a valid source ' do
      content = catalogue.resource('file', 'tftp.conf').send(:parameters)[:source]
      content.should == "puppet://modules/tftp/spec"
    end
    it 'should request a valid source dir' do
      content = catalogue.resource('file', 'tftp.dir').send(:parameters)[:source]
      content.should == "puppet://modules/tftp/dir/spec"
    end
    it 'should purge source dir if source_dir_purge is true' do
      content = catalogue.resource('file', 'tftp.dir').send(:parameters)[:purge]
      content.should == true
    end
  end
  ...
  describe 'Test service autorestart' do
    it 'should automatically restart the service, by default' do
      content = catalogue.resource('file', 'tftp.conf').send(:parameters)[:notify]
      content.should == "Service[tftp]"
    end
  end
  ...
  describe 'Test Monitoring Tools Integration' do
    ...
    it 'should generate monitor defines' do
      content = catalogue.resource('monitor::process', 'tftp_process').send(:parameters)[:tool]
  ...
  describe 'Test params lookup' do
    let(:facts) { { :tftp_monitor => true , :ipaddress => '10.42.42.42' } }
    let(:params) { { :port => '42' } }
  ...
  describe 'Test params lookup' do
    let(:facts) { { :monitor => false , :tftp_monitor => true , :ipaddress => '10.42.42.42' } }
    let(:params) { { :port => '42' } }
